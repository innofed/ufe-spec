...
此规范意在提高`Markdown`文档的可读性。


## 规则

* 后缀 **[必须]** 使用 `.md`。
* 文件名 **[必须]** 使用小写，多个单词之间使用`-`分隔。
* 文件编码 **[必须]** 用 UTF-8。
* 章节标题 **[必须]** 以 `##` 开始，而不是 `#`。
* 章节标题 **[必须]** 在 `#` 后加一个空格，且后面没有 `#`。
...
* 代码段 **[必须]** 使用 Fenced code blocks 风格，如下所示：

        ```
        console.log("");
        ```

* 表格的写法 **[应该]** 参考 [GFM](https://help.github.com/articles/github-flavored-markdown)，如下所示：
...
* 表达方式 **[应该]**  遵循《The Element of Style》：
